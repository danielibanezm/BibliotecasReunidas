...
public class Insertar_Libro extends JDialog {
	...
	public static void main(String[] args) {
		try {
			Insertar_Libro dialog = new Insertar_Libro();
		...
		contentPanel.add(txtUbicacion);
		{
				...
				btnGuardar.addMouseListener(new MouseAdapter() {
					...
					public void mouseClicked(MouseEvent e) {
						...
						if (!(txtIsbn.getText().isEmpty() || txtTitulo.getText().isEmpty()
				                ...
				                || txtPaginas.getText().isEmpty() || txtGenero.getText().isEmpty() || txtUbicacion.getText().isEmpty())) {
			...
			}
			{
				...
				btnCancelar.addMouseListener(new MouseAdapter() {
					@Override
					public void mouseClicked(MouseEvent e) {
						txtIsbn.setText("");txtTitulo.setText("");txtAutores.setText("");txtGenero.setText("");
						txtEditorial.setText("");txtIdioma.setText("");txtEdicion.setText("");txtPublicacion.setText("");
						txtPais.setText("");txtPaginas.setText("");txtUbicacion.setText("");txtUbicacion.setText("");
	...
	public void insertar(Libros nuevoLib) {
		String ultimoId = "", nuevoId = "";
		int opcion = 0;
		
		ultimoId = bd.obtenerUltimoIdLibro();
		System.out.println("ultimo id = " + ultimoId);
				
		if((nuevoId = obtenerSiguienteIdLibro(ultimoId)) != null ) {
			System.out.println("nuevo id = " + nuevoId);
			opcion = err.preguntarInsertar();
			
			if(opcion == 0) {
				bd.insertarLibro(nuevoId, nuevoLib);
	...
	public String obtenerSiguienteIdLibro(String ultimoId) {
		int num;
		String nuevoId;
		
	    if (ultimoId != null && ultimoId.startsWith("L")) {
	        try {
	            num = Integer.parseInt(ultimoId.substring(1));
	            num ++;
	            nuevoId = "L" + Integer.toString(num);
	    ...
	    //Se retorna null si hay algún error.
	    return null;
	}
